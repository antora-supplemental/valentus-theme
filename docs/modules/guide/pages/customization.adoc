...
Valentus currently ships an *interim* paint-hold in `partials/head-meta.hbs` (`html.adt-fouc-pending`) marked pending upstream Default UI guidance / !207 — if you override `head-meta.hbs`, keep that hold (stylesheets first, then preference scripts, then release) or you reintroduce the flash.
