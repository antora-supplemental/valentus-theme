...
== Nav expand / collapse broken after override

*Symptom:* Sidebar carets do nothing, or expand then immediately collapse; console may show duplicate listeners.

*Cause:* Hub `footer-content.hbs` (or another supplemental partial) reloads `site.js` / Valentus footer scripts while theme `footer-scripts.hbs` already injects them.

*Fix:* Remove the duplicate `<script src="…/js/site.js">` (and duplicate Valentus script tags). Keep a single load via `footer-scripts.hbs`; append only hub companions (e.g. `site-nav-tree-current.js`) after it. See xref:customization.adoc[Customization].
...
== Wide table spills over the TOC

*Symptom:* A wide table extends past the article column and covers the right-hand TOC (or widens the whole page).

*Checks:*

. Confirm `site-visual.css` and `adt-table-scroll.js` load from theme `footer-scripts.hbs` (or the `doc-layout` module foot bundle). Hub overrides must not drop the table-scroll script.
. In DevTools, the table should sit inside `.tablecontainer` or `.adt-table-scroll` with `overflow-x: auto` on that wrapper—not on the `<table>`.
. Nested tables inside admonitions or column lists are intentionally not re-wrapped; narrow those tables in AsciiDoc or split content.

See link:content-viewport.adoc[Content viewport and wide tables].
