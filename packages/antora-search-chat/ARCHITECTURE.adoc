...
* Own the centered navbar search slot (`.adt-navbar-search` / `.adt-nav-search`). Page read-width stays in `.adt-tool-band` over the doc column. Search width is independent of `--adt-doc-max` / nav column measure.
...
The omnibox (compound scope + field + results) lives in `.adt-site-navbar` (`.adt-navbar-search`) as a site-wide command palette. Its width is independent of `--adt-doc-max` and the nav column. Page read-width stays in `.adt-tool-band` centered over the doc column. Prior “search over nav / resize over doc” in the tool band inverted that model for a command-palette search — corrected in 2.0.1. Search-chat owns the control; layout owns the unclipped navbar slot.
