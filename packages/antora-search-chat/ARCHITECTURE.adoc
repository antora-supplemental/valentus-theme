...
Placeholder copy:: *Search or Ask* when Ask is on; *Search* + muted *(AI mode not configured)* when off (see <<_fake_placeholder_overlay>>).

Do not put a second search box in the tool band or nav column while this navbar slot is active.

==== Fake placeholder overlay

Native `<input placeholder>` is a single unstyled string, so the compound control uses an absolutely positioned overlay (`.adt-search-ph`):
...
* *Ask enabled* — copy is *Search or Ask* (`.adt-search-ph-main` only).
* *Ask missing / disabled* — *Search* in the normal placeholder color + *(AI mode not configured)* in a deader tint (`.adt-search-ph-note`).
...
* Native `placeholder` stays empty so it never fights the overlay; `aria-label` matches the visible meaning.
...
* Class hooks: `.adt-search-compound` wraps `.adt-search-scope-slot` (full-height fill + `.adt-search-scope`) + `.adt-search-input-wrap` (`#search-input` + `.adt-search-ph` overlay).
...
* Scope: conjoined dropdown on the *left edge* of the field (`.adt-search-compound` / `.adt-search-scope-slot`) — *All* / *This component* / optional *This version*; defaults per <<_default_scope_recommendation>>.
...
| `ask_enabled`
| boolean
| Explicit Ask / AI omnibox affordance. When `true`, placeholder is *Search or Ask*; when `false` or unset without backend/local assist, *Search* + muted *(AI mode not configured)*. Aliases: `askEnabled`.
...
Valentus layout may still fall back to a plain lunr field (`placeholder="Search"`) when `SITE_SEARCH_CHAT` is unset. The fake placeholder overlay ships in this package (`adt-search-chat` partial + `site-search-chat.css` / `.js`). Themes that keep only `#search-input` should either:

* Prefer `{{> adt-search-chat}}` when the extension is registered, or
* Copy the `.adt-search-ph` overlay and drive Ask-on/off copy from `site.keys.askEnabled` / extension config.
