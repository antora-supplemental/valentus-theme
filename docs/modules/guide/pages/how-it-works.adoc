...
=== 1. head-meta.hbs (FOUC Prevention + CSS chain)

This Handlebars partial is injected into the `<head>` of every page.
It:

. Applies the interim paint-hold (`html.adt-fouc-pending`)
. Loads the Valentus stylesheet chain (`site-extra`, `site-doc-layout`, `site-visual`, `site-read-width`, `site-font-size`, `site-math`)
. Runs preference FOUC scripts (dark mode, read-width, font-size) and releases paint after webfonts

Hub overrides must keep that full chain — see xref:customization.adoc[Customization].
Simplified illustration (not the full production partial):
...
The preference script runs before the page paints so dark mode does not flash light-first.
...
* The main Antora site script (`site.js`)
* highlight.js for syntax highlighting
* Valentus scripts (dark mode, VCS icons, accordion, visual, read-width, font-size, math, …)

Hubs that override footer partials must **not** load `site.js` (or these Valentus scripts) a second time — duplicate handlers break nav expand/collapse.
Add only hub-specific companions after the theme list (see xref:customization.adoc[Customization]).
